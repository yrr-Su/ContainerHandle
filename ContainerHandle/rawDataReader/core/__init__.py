...
class _reader:
    ...
    def loggerCreater(self) -> None:
        ...
        _formatter = logging.Formatter(_formate, datefmt='%Y-%m-%d %X')


        file_handler = logging.FileHandler(
            self.path_trouble / f'{self.nam}_debug.log',
            mode='w'
            )

        trouble_file_handler = logging.FileHandler(
            self.path_trouble / f'{self.nam}_error.log',
            mode='w'
            )
        ...
        trouble_file_handler.setLevel(logging.ERROR)
        trouble_file_handler.setFormatter(_formatter)

        if self.logger.handlers:
            self.logger.handlers.clear()

        self.logger.addHandler(file_handler)
        self.logger.addHandler(trouble_file_handler)
