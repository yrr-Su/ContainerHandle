...
from pandas import DataFrame, to_datetime, concat, Series, DatetimeIndex, to_numeric
...
def _powerlaw_fit_dN(_smps, _aps, _alg_type):
	...
	shift_factor_dN = DataFrame(_shift_factor.loc[_dropna_idx].values[range(len(_dropna_idx)), S2.loc[_dropna_idx].idxmin(axis=1).values],
...
def _corr_fc(_aps_dia, _smps_dia, _smps_dn, _aps_dn, _smooth, _idx, _sh):	
	...
	S2_lst = []
	for (_tm, _inp_y_dn), (_tm, _cor_y_dn) in zip(input_y.dropna(how='all').iterrows(), corr_y.dropna(how='all').iterrows()):

		## corr(spec_data, spec_spline)
		_spl_dt = [ unvpline(input_x, _inp_y, s=_smooth)(corr_x) for _inp_y in [_inp_y_dn, ds_fc(_inp_y_dn), dv_fc(_inp_y_dn)] ]
...
def _corr_with_dNdSdV(_smps, _aps, _alg_type):
	...
	S2_lst = pool.starmap(partial(_corr_fc, _aps_dia, _smps_dia, _smps_dn, _aps_dn, _smooth), list(enumerate(_shift_val)))
...
def merge_SMPS_APS(df_smps, df_aps, df_pm25, aps_unit='um', smps_overlap_lowbound=500, aps_fit_highbound=1000, 
	...
	for (_typ, _lst), (_typ, _shft) in zip(fitness_typ.items(), shift_typ.items()):
		...
		df_shift = concat(_shft, axis=1, keys=times_ary.tolist()).loc[df_times_min.index].values[range(len(df_times_min.index)), df_times_min.values]
