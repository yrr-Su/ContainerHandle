...
def _overlap_fitting(_smps_ori,_aps_ori,_smps_lb,_aps_hb):
	...
	## coefficient A, B
	_smps_qc_cond = ((_smps!=0)&n.isfinite(_smps))
	...
	_aps_shift_x = _aps_shift_x.where(n.isfinite(_aps_shift_x))
...
def _merge_data(_smps_ori,_aps_ori,_shift_ori,_shift_mode,_smps_lb,_aps_hb,_coe):
	...
	_uni_idx, _count = n.unique(n.hstack((_smps_ori.dropna(how='all').index,_aps_ori.dropna(how='all').index,
							    _shift_ori.dropna(how='all').index)),return_counts=True)
								
	_merge_idx = to_datetime(n.unique(_uni_idx[_count==3]))
	...
	for _bin_smps, _bin_aps, _dt_smps, _dt_aps, _sh in zip(_smps_bin,_aps_bin,_smps.values,_aps.values,_shift):
		...
		__merge = n.exp(_unvpl_fc(n.log(_std_bin_merge)))
